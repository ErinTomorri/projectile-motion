 ...
 */
public class Main {
    public static void write(Projectile projectile[], int numPeople, int count) { //this is the wrting function this will write over the txt file with the new text
        NumberFormat decimal = NumberFormat.getNumberInstance();
        try {
            FileWriter writer = new FileWriter("text.txt"); // finds the text file
            String temp = "Names, Distance from dummy, Angle, Velocity, Gravity, Height \n"; // allocates the temp with a the title and coloums of the menus
            for (int i = 0; i < numPeople + count+1; i++) {
                // this gets all the data such as names, dummy distance, angle, velocity and etc and stores it to the end of temp
                temp = temp + i; 
                temp = temp + "." + projectile[i].getNames() + "," + projectile[i].getDummy() + "," + decimal.format(projectile[i].getAngle()*180/Math.PI) + "," + projectile[i].getVelocity() + "," + projectile[i].getAcceleration() + "," + projectile[i].getHeight() + "\n";
            }
    ...
    public static void read(int count, Projectile projectile[]) { // reads the file and allocates the projectiles arrays filled with the text file
        String prev;
        NumberFormat decimal = NumberFormat.getNumberInstance();
        try {
            File myObj = new File("text.txt"); //opens the text file
            ...
            do { // a do while loop to check if the user wants to check their previous games
                ...
                while (myReader.hasNextLine()) { // checks if theres a next line in the file
                    int a = 0;
                    double b = 0;
                    String c = "";
            
                    for (int j = 0; j <= count + 1; j++) { // goes through all the file lines
                        if (j == 0) { // if j is 0 skip the line basically because its the title
                            if (myReader.hasNextLine()) {
                                data = myReader.nextLine();
                            }
                            
                        } else {//extract all the data and puts them in corresponding variables 
                            data = data.substring(data.indexOf('.') + 1);
                            String name = (data.substring(0, data.indexOf(','))); //saves the name of the projectile into name variable
                            data = data.substring(data.indexOf(',') + 1);
            
                            double dummyDistance = Double.parseDouble(data.substring(0, data.indexOf(','))); // saves the dummy distance into a double
                            data = data.substring(data.indexOf(',') + 1);
            
                            double angle = Double.parseDouble(data.substring(0, data.indexOf(','))); // saves the angle into a double
                            data = data.substring(data.indexOf(',') + 1);
            
                            double velocity = Double.parseDouble(data.substring(0, data.indexOf(','))); // save the velocity into a double
                            data = data.substring(data.indexOf(',') + 1);
            
                            double grav = Double.parseDouble(data.substring(0, data.indexOf(','))); // save the gravity into a double 
                            data = data.substring(data.indexOf(',') + 1);

                            double height = Double.parseDouble(data); //save the height into a double
            
                            projectile[j-1] = new Projectile(name, height, velocity, angle, dummyDistance); //stores the value into a new projectile class
                            if (height>0){
                                ...
                                projectile[j-1].setTimeThrow();
                                projectile[j-1].setDistanceXthrow();
                                projectile[j-1].setDistanceYthrow();
                                projectile[j-1].setDistanceFromDummyThrow();
                            }
                ...
                if (prev.equals("y")) {
                    ...
                    for (int i = 0; i<count+1; i++){
                        System.out.println(i + 1 + "." + projectile[i].getNames() + "," + projectile[i].getDummy() + "," + decimal.format(projectile[i].getAngle()*180/Math.PI) + "," + projectile[i].getVelocity() + "," + projectile[i].getAcceleration() +","+ projectile[i].getHeight()+ "\n");
                    }
                }
                myReader.close(); 
    ...
    public static void main(String[] args) {
        ...
        while (checkPlayAgain) { // determine if you kick or throw the ball
            ...
            int count = count();
            players = new Projectile[numPeople + count+1];
            ...
            if (action.equals("kick")) {
                ...
                for (int i = count; i < numPeople+count; i++) {
                    ...
                    players[i+1] = new Projectile(name, velocity, angle, dummyDistance);
                    players[i+1].setDistanceXkick(); // davit check over to see if we can make this better
                    players[i+1].setDistanceYkick(); // it works but might lose us marks
                    players[i+1].setTimekick();
                    players[i+1].setDistanceFromDummyKick();
                    players[i+1].setAcceleration(grav);
                ...
                if (players.length == 1) {
                    ...
                    for (int i = 0; i < numPeople+count+1; i++) { // Display users in order from closest to furthest and
                ...
                for (int i = count; i < numPeople+count; i++) {
                    ...
                    players[i+1] = new Projectile(name, height, velocity, angle, dummyDistance);
                    players[i+1].setTimeThrow();
                    players[i+1].setDistanceXthrow();
                    players[i+1].setDistanceYthrow();
                    players[i+1].setDistanceFromDummyThrow();
